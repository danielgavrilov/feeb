...
const LEGACY_CHILD_ALLERGEN_ALIASES: Record<string, string[]> = {
...
export const IngredientsList = ({
  ...
  const matchesAllergenSelection = (
    ...
    if (normalizedId.includes(":")) {
      const [, memberId = ""] = normalizedId.split(":");

      if (
        (normalizedCode?.includes(":") && normalizedCode.split(":")[1] === memberId) ||
        (normalizedName?.includes(":") && normalizedName.split(":")[1] === memberId)
      ) {
        return true;
      }

      const childAliases = LEGACY_CHILD_ALLERGEN_ALIASES[normalizedId] ?? [];

      if (
        (normalizedCode && childAliases.includes(normalizedCode)) ||
        (normalizedName && childAliases.includes(normalizedName)) ||
        (normalizedFallback && childAliases.includes(normalizedFallback))
      ) {
        return true;
      }
